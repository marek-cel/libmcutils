...
{
    mc::MovingAverage ma;
    EXPECT_EQ( ma.getLength(), 1 );
    EXPECT_NEAR( ma.getValue(), 0.0, 1.0e-9 );
}
...
TEST_F(TestMovingAverage, CanInstantiateAndSetData)
{
    ...
    EXPECT_EQ( ma.getLength(), 5 );
    EXPECT_NEAR( ma.getValue(), 1.0, 1.0e-9 );
}
...
{
    mc::MovingAverage ma;
    EXPECT_NO_THROW( ma.setLength( 5 ) );
    EXPECT_EQ( ma.getLength(), 5 );
}
...
TEST_F(TestMovingAverage, CanUpdate)
{
    ...
    ma.update( 1.0, u[ 0 ] );
    // 44
    EXPECT_DOUBLE_EQ( ma.getValue(), 44.0 );

    ma.update( 1.0, u[ 1 ] );
    // ( 44 + 64 ) / 2 = 54
    EXPECT_DOUBLE_EQ( ma.getValue(), 54.0 );

    ma.update( 1.0, u[ 2 ] );
    // ( 44 + 64 + 18 ) / 3 = 42
    EXPECT_DOUBLE_EQ( ma.getValue(), 42.0 );

    ma.update( 1.0, u[ 3 ] );
    // ( 44 + 64 + 18 + 65 ) / 4 = 47.75
    EXPECT_DOUBLE_EQ( ma.getValue(), 47.75 );

    ma.update( 1.0, u[ 4 ] );
    // ( 44 + 64 + 18 + 65 + 81 ) / 5 = 54.4
    EXPECT_DOUBLE_EQ( ma.getValue(), 54.4 );

    ma.update( 1.0, u[ 5 ] );
    // ( 64 + 18 + 65 + 81 + 28 ) / 5 = 51.2
    EXPECT_DOUBLE_EQ( ma.getValue(), 51.2 );

    ma.update( 1.0, u[ 6 ] );
    // ( 18 + 65 + 81 + 28 + 1 ) / 5 = 38.6
    EXPECT_DOUBLE_EQ( ma.getValue(), 38.6 );

    ma.update( 1.0, u[ 7 ] );
    // ( 65 + 81 + 28 + 1 + 25 ) / 5 = 40
    EXPECT_DOUBLE_EQ( ma.getValue(), 40.0 );

    ma.update( 1.0, u[ 8 ] );
    // ( 81 + 28 + 1 + 25 + 41 ) / 5 = 35.2
    EXPECT_DOUBLE_EQ( ma.getValue(), 35.2 );

    ma.update( 1.0, u[ 9 ] );
    // ( 28 + 1 + 25 + 41 + 39 ) / 5 = 26.8
    EXPECT_DOUBLE_EQ( ma.getValue(), 26.8 );
}
